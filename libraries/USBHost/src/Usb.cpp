...
uint32_t USBHost::SetPipeAddress(uint32_t addr, uint32_t ep, EpInfo **ppep, uint32_t &nak_limit) {
	...
	*ppep = getEpInfoEntry(addr, ep);
    ...
    if (ep) {
#if 0
		SERIAL_PORT_MONITOR.print("UHD_Pipe_Alloc(");
		SERIAL_PORT_MONITOR.print(addr); SERIAL_PORT_MONITOR.print(',');
		SERIAL_PORT_MONITOR.print(ep); SERIAL_PORT_MONITOR.print(',');
		SERIAL_PORT_MONITOR.print((*ppep)->bmAttribs); SERIAL_PORT_MONITOR.print(',');
		SERIAL_PORT_MONITOR.print((*ppep)->epAddr & 0x80); SERIAL_PORT_MONITOR.print(',');
		SERIAL_PORT_MONITOR.print((*ppep)->maxPktSize); SERIAL_PORT_MONITOR.println(')');
#endif
